...
- Реальні зображення товарів віддаються через бекенд‑проксі

## Мок-логін/реєстрація (як у frontend2)
...
- Проксі зображень (щоб не було 403/хотлінку)
  - `GET /proxy?url={absolute_image_url}`

## Важливо

- Фронт звертається до бекенду на `http://127.0.0.1:8000/api`. За потреби можна задати `VITE_API_URL`.
- Зображення в каталозі/товарах проходять через бекенд‑проксі (`/api/proxy?...`) — так стабільніше.
...
`backend/app/seed.py` — бренди/категорії/товари з описами та реальними зображеннями. Можна доповнювати список товарів або замінювати посилання на власні.
